...
> **Cảnh báo:** seed chạy `TRUNCATE ... RESTART IDENTITY CASCADE` trên toàn bộ 10 bảng trước khi chèn dữ liệu. Chỉ dùng cho development/test; không dùng trên database có dữ liệu cần giữ lại. Hash mật khẩu và URL trong seed đều là dữ liệu mẫu.
...
ERD kèm danh sách cột đầy đủ: [database/nomad-diary-er.mmd](database/nomad-diary-er.mmd).
...
- Xóa thật user sẽ cascade tới trip và các dữ liệu phụ thuộc của trip.
...
Seed hiện có một người dùng, 6 chuyến đi, 10 tỉnh/thành phố, 24 địa điểm, 24 lần ghé, 21 review, 10 tag và 64 ảnh. Dữ liệu minh hoạ các trường hợp ghé lại địa điểm qua nhiều chuyến, ghé cùng một nơi nhiều lần trong một chuyến, ảnh cấp trip/stop và các trạng thái đánh giá khác nhau.

## Phạm vi hiện tại

Repository hiện chỉ chứa thiết kế database và dữ liệu mẫu; chưa có API, ứng dụng web, Docker configuration hay chính sách phân quyền/RLS. Các tính năng như chi phí, chặng di chuyển, chỗ ở, bạn đồng hành và khoảng cách thực tế chưa thuộc mô hình hiện tại.
